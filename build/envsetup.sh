function __print_pear_functions_help() {
cat <<EOF
Additional PearOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- peargerrit:      A Git wrapper that fetches/pushes patch from/to PearOS Gerrit Review.
- pearrebase:      Rebase a Gerrit change and push it again.
- pearremote:      Add git remote for PearOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for PearOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Pear model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch pear_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/pear-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.pear.device | grep -q "$PEAR_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $PEAR_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function pearremote()
{
    ...
    local PEAR_USER=$(git config --get review.review.pearos.org.username)
    if [ -z "$PEAR_USER" ]
    then
        git remote add pear ssh://review.pearos.org:29418/$PFX$PROJECT
    else
        git remote add pear ssh://$PEAR_USER@review.pearos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'pear' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/PearOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.pear.device | grep -q "$PEAR_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        pearremote
        git push pear HEAD:refs/heads/'$1'
    fi
    '
}
...
function peargerrit() {
    ...
    local user=`git config --get review.review.pearos.org.username`
...
function pearrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "PearOS Gerrit Rebase Usage: "
        echo "      pearrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.pearos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.pear.device | grep -q "$PEAR_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/pear/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $PEAR_FIXUP_COMMON_OUT ]; then
