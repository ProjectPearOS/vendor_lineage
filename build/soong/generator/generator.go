...
func init() {
	android.RegisterModuleType("pear_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = pearExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = pearExpandVariables(ctx, dep_file)
	...
	cmd := pearExpandVariables(ctx, String(g.properties.Cmd))
